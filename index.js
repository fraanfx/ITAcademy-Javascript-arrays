...
function buscarVocal (nombree) {
 ...
 //Ex 3 2
 function numLetras2(palabra){
   var object2 = {}
   var letrasRepetidas = []
   palabra.map(function(letra){
    let char = object2[letra]
    let repetida = false;
    for(let j = 0; j < letrasRepetidas.length; j++){
      if (letrasRepetidas[j] == char){
        repetida = true;
        console.log('entro');
      }
    ...
    for (let i =0 ; i < object2.length; i++){
      ...
      if(object2[i] == char){
        ...
        counterChar++;
        object2[letra] + counterChar;
        letrasRepetidas.push(object2[letra]) 
      }
      else{
        console.log('aqui tb')
        object2[letra] = +1;
